...
namespace FilterRow
{
	...
	public class FilterRow
	{
        ...
        {
            _flex.Select(_row, nColToSelect);
        ...
        private void _flex_ChangeEdit(object sender, EventArgs e)
        {
            if (_flex.Row == _row)
            {
                Console.WriteLine("_flex_ChangeEdit: " + _flex.Editor.Text);
		...
		private void UpdateFilter()
		{
            ...
            try{
			  ...
			  //  cm.EndCurrentEdit();
                ...
                for (int col = _flex.Cols.Fixed; col < _flex.Cols.Count; col++)
                {
                    // get column value
                    string expr = null;
                    if (col != _flex.ColSel || _flex.Editor==null)
                    {
                        expr = _flex.GetDataDisplay(_row, col).TrimEnd();
                    }
                    else
                    {
                        expr = _flex.Editor.Text;
                    }
			...
			{
				_flex[_row, 0] = "Err";
                Console.WriteLine("Bad filter expression.");
			}
